...
export const Sidebar: React.FC<SidebarProps> = ({ isOpen, onClose, activeRoute = '/' }) => {
  const navItems = [
    { label: 'Dashboard', icon: 'grid_view', path: '/' },
    { label: 'Antrean Pasien', icon: 'groups', path: '/queue' },
    { label: 'Konsultasi Dokter', icon: 'stethoscope', path: '/consultations' },
    { label: 'Tagihan Kasir', icon: 'payments', path: '/invoices' },
    { label: 'Farmasi & Obat', icon: 'medication', path: '/pharmacy' },
    { label: 'Data Dokter', icon: 'badge', path: '/doctors' },
  ];
      ...
      >
        <div>
          ...
          <div className="space-y-2">
            {navItems.map((item) => {
              const isActive = activeRoute === item.path || (item.path === '/' && activeRoute === '/');
        ...
        <div className="pt-4 border-t-2 border-[#18181b] space-y-2">
          ...
          <a
            href="/login"
            className="flex items-center gap-3 px-4 py-2.5 rounded-xl text-xs font-black text-white bg-rose-500 border-2 border-[#18181b] hover:bg-rose-600 transition-colors shadow-[2px_2px_0px_#18181b]"
          >
      ...
      <aside className="hidden md:flex fixed left-4 top-1/2 -translate-y-1/2 w-14 py-3.5 rounded-[32px] bg-white border-2 border-[#18181b] shadow-[4px_4px_0px_#18181b] flex-col items-center justify-between z-40 gap-2.5 max-h-[85vh] overflow-y-auto scrollbar-none">
        ...
        <div className="flex flex-col gap-1.5 items-center w-full pt-2 border-t-2 border-[#18181b]/10 shrink-0">
          ...
          <a
            href="/login"
            title="Logout"
            className="w-8 h-8 rounded-full flex items-center justify-center text-[#52525b] hover:bg-rose-100 hover:text-rose-600 transition-colors"
          >
