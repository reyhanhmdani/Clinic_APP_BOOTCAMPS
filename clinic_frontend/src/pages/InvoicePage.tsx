    ...
    <div className="organic-bg min-h-screen w-full p-4 sm:p-6 md:p-8 text-[#18181b] font-sans antialiased">
      <div className="max-w-4xl mx-auto space-y-6">
          ...
          <div className="flex items-center gap-3.5">
            ...
            <div>
              ...
              <p className="text-xs font-bold text-[#52525b]">
                Proses pelunasan nota dan cetak bukti transaksi resmi klinik
              </p>
            </div>
          </div>

          <span
            className={`text-[#18181b] border-2 border-[#18181b] font-black text-xs px-3.5 py-1.5 shadow-[3px_3px_0px_#18181b] uppercase tracking-wider ${
        ...
        <div className="grid grid-cols-1 md:grid-cols-12 gap-6 items-start">
          ...
          <div className="md:col-span-7 p-6 bg-white border-3 border-[#18181b] shadow-[5px_5px_0px_#18181b] space-y-5">
            {/* Header Struk */}
            <div className="flex justify-between items-start pb-4 border-b-2 border-[#18181b]">
              <div>
                <h2 className="text-lg font-black text-[#18181b] uppercase">ReyClinic Outpatient Receipt</h2>
                <p className="text-xs font-bold text-[#52525b]">
                  No. Faktur: <b className="text-[#18181b] font-mono">{activeVisit?.invoice?.invoiceNo || '-'}</b>
                </p>
              </div>
              <div className="text-right">
                ...
                <p className="text-[11px] font-bold text-[#52525b] uppercase">Kasir: Rey Admin</p>
              </div>
            </div>

            {/* Info Pasien & Dokter */}
            <div className="p-3.5 bg-[#fef08a] border-2 border-[#18181b] shadow-[3px_3px_0px_#18181b] flex justify-between items-center text-xs">
              <div>
                <p className="font-black text-[#18181b] uppercase">
                  Pasien: {activeVisit?.patient?.name || (isLoading ? 'Memuat...' : '-')}
                </p>
                <p className="font-bold text-[#52525b]">RM: {activeVisit?.patient?.noRm || '-'}</p>
              </div>
              <div className="text-right font-black text-[#18181b] uppercase">
                Dokter: {activeVisit?.doctor?.name || '-'}
              </div>
            </div>
            ...
            <div className="space-y-2.5">
              <h3 className="text-xs font-black uppercase text-[#18181b] tracking-wider mb-2">
                Rincian Layanan & Resep Obat
              </h3>

              {/* Jasa Konsultasi */}
              <div className="flex justify-between items-center py-2.5 border-b border-[#18181b]/10 text-xs">
                <div>
                  <span className="font-black text-[#18181b] block">Jasa Konsultasi Medis</span>
              ...
              <div className="flex justify-between items-center py-2.5 border-b border-[#18181b]/10 text-xs">
                <div>
                  <span className="font-black text-[#18181b] block">Total Resep Obat Apotek</span>
                  <span className="text-[10px] font-bold text-[#52525b]">Farmasi / Obat</span>
                </div>
            ...
            <div className="p-4 bg-[#a3e635] border-3 border-[#18181b] shadow-[4px_4px_0px_#18181b] flex justify-between items-center">
          ...
          <div className="md:col-span-5 p-6 bg-white border-3 border-[#18181b] shadow-[5px_5px_0px_#18181b] space-y-5">
            <h3 className="text-xs font-black text-[#18181b] uppercase tracking-wider flex items-center gap-2 pb-2 border-b-2 border-[#18181b]/10">
            ...
            <div className="space-y-2">
              {[
                ...
                { id: 'CARD', label: 'Kartu Debit / EDC', icon: 'credit_card', color: 'bg-[#f472b6]' },
              ].map((method) => (
                <button
                  key={method.id}
                  type="button"
                  onClick={() => setPaymentMethod(method.id as any)}
                  className={`w-full p-3 border-2 border-[#18181b] text-left transition-all flex items-center gap-3 cursor-pointer ${
                    paymentMethod === method.id
                      ? `${method.color} text-[#18181b] font-black shadow-[3px_3px_0px_#18181b] scale-[1.02]`
                      : 'bg-white text-[#18181b] font-bold hover:bg-[#fde047]/30'
                  }`}
                >
                  <span className="material-symbols-outlined text-[20px]">{method.icon}</span>
                  <span className="text-xs font-black flex-1 uppercase tracking-wider">{method.label}</span>
                  {paymentMethod === method.id && (
            ...
            {/* Action Buttons */}
            <div className="pt-4 border-t-2 border-[#18181b]/10 space-y-2.5">
              <button
                type="button"
                disabled={isProcessing || isAlreadyPaid || !activeVisit?.invoice?.id}
                onClick={handlePayInvoice}
                className="w-full py-3.5 neubrutal-btn-primary text-xs font-black text-[#18181b] border-2 border-[#18181b] cursor-pointer shadow-[3px_3px_0px_#18181b] hover:scale-102 active:translate-y-0.5 transition-all flex items-center justify-center gap-2 disabled:opacity-50 uppercase tracking-wider"
              >
              ...
              <button
                ...
                className="w-full py-2.5 border-2 border-[#18181b] bg-white text-xs font-black text-[#18181b] hover:bg-zinc-100 transition-all shadow-[2px_2px_0px_#18181b] flex items-center justify-center gap-2 cursor-pointer uppercase tracking-wider"
              >
